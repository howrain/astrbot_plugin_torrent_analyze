...
如果图片中的中文显示为方框（口口口），通常是容器里没有可用的中文字体。

建议：

1. 在运行环境安装 CJK 字体（例如 `fonts-noto-cjk`）。
2. 必须下载并放置指定字体文件：`MapleMono-CN-Regular.ttf`。
3. 放置目录固定为：`/AstrBot/data/fonts/MapleMono-CN-Regular.ttf`。
4. 字体下载地址（官方）：<https://github.com/subframe7536/maple-font/releases/latest>。
5. 完成后重启 AstrBot 或热重载插件。
